...
Algorithm::Algorithm(string& _init, string& _alloc) {
...
void Algorithm::run() {
	...
	int numIters = 1500;
	whenFail = {  350 }; // Simulation step when failure occurs, could be randomized
	whoFail = {  1 }; // ID of the robot that will experience failure, could be randomized
...
bool Algorithm::condition4(vector<int>& whichBotsOperGlobal, vector<bool>& operBotsGlobal) {
...
void Algorithm::findIntersections() {
...
void Algorithm::ifConnected(bool _connected, vector<int>& bins, int i) {
...
void Algorithm::updateParticles() {
...
void Algorithm::move_particle_forces(Robot& robot, Robot& particle) {
	...
	theta_d = copysign(min(numbers::pi, abs(theta_d)), theta_d);
...
void Algorithm::move_robot_forces(Robot& robot) {
	...
	theta_d = std::copysign(1.0, theta_d) * std::min(std::numbers::pi, std::abs(theta_d));
...
Point Algorithm::move_robot_forces_sim(Robot& robot, double obsRho) {
	vector<Vector2d> obs; // To store (column, row) pairs
	//	auto goal = particle.goalLoc;
	// Find indices where robot.M0 > 0.5 and convert them to (column, row)
	// double obsRho = robot.obsDist[0];
...
void Algorithm::failingMechanism(vector<int>& whenFail, vector<int>& whoFail, vector<bool>& operBotsGlobal, int howFail, int i) {
	for (int k = 0; k < numBots; k++) {
...
void Algorithm::task_check_mcts_all(vector<int>& conBots, vector<int>& opBots, vector<int>& bins, int step, int k) {
...
void Algorithm::replan_check_mcts(vector<int>& conBots, vector<int>& opBots, int step, int k, MCTS& mcts) {
...
void Algorithm::goal_check_mcts(int step, int k, double goalReached) {
...
void Algorithm::findIntersectionPoint(Robot& robot, Pose& locB, Robot pB, double velA, double velB, double goalReached, int step) {
	double radius = 0;
	//int taskCount = 1;
	int taskCount = 0;
...
void Algorithm::findMinTimes(int step, int k, vector<int>& conBots, vector<int>& deadBots, set<int>& botsComm, vector<vector<double>>& rzLoc, vector<double>& minTimes) {
...
int Algorithm::computePartition(taskDict& newTaskDict, vector<double>& timeForTask, vector<Pose>& new_loc, vector<double>& cost, vector<int>& opBots, int tNew, int k) {
	vector<int> taskLeft;
	Pose task;
...
double Algorithm::computeTourLength(Robot& robot, Pose& pose, vector<int>& tour) {
	double time = 0;
...
void Algorithm::insertPlan(Robot& robot, vector<Plan>& whichTasks) {
...
void Algorithm::updateGoal(Robot& robot, int step) {
...
void Algorithm::update_robots(Robot& robot, PResult& planningResults, vector<Plan>& whichTasks, vector<int>& opBots, vector<int>& conBots, vector<int>& deadBots, int ops)
{
...
void Algorithm::update_robots2(Robot& robot, vector<int>& deadBots, vector<int>& conBots)
{
...
void Algorithm::replanOriginal(vector<int>& conBots, vector<int>& opBots, int step, int k, MCTS& mcts) {
