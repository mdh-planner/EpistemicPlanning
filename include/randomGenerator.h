...
template<typename T> T
inline getRandomRealInRange(T minInclusive, T maxInclusive) // T must be either double, double or long double. int wont work.
{
	if (minInclusive > maxInclusive) {
		return -1;
	}
...
//
//	//return _vector;
//	std::vector<T> shuffled(temp); // Create a copy for shuffling to preserve 'temp'
//	std::mt19937 rng(std::random_device{}()); // Properly initialized Mersenne Twister
//	std::shuffle(shuffled.begin(), shuffled.end(), rng); // Shuffle the copy
//
//	_vector.clear();
//	_vector.insert(_vector.end(), shuffled.begin(), shuffled.end()); // Assign shuffled vector
//
//	return _vector;
//}

template<typename T>
void getRandomIntVectorFromOtherVector(const std::vector<T>& temp, std::vector<T>& _vector)
{
	std::vector<int> shuffled(temp); // Create a copy for shuffling
	auto rng = getMersenneTwisterEngine(); // Use your defined function to get the RNG
	std::shuffle(shuffled.begin(), shuffled.end(), *rng); // Shuffle the copy

	_vector = shuffled; // Assign shuffled vector directly
}
