...
namespace mdutec {
	...
	public:
		...
		Algorithm(std::string inPathToLoggingDirectory = "D:\\LOGS\\ECTSP") {
		...
		virtual DistMat calculateDistances(std::vector<City>& inCityData, std::vector<Salesperson>& inAgentData) {
			...
			for (size_t i = 0; i < inCityData.size(); i++)
			{
				...
				for (size_t j = 0; j < inCityData.size(); j++)
				{
					...
					_result[i][j] = sqrt(
						(ith_city.first - jth_city.first) * (ith_city.first - jth_city.first)
						+ (ith_city.second - jth_city.second) * (ith_city.second - jth_city.second));// / inAgentData[0].getSpeed(); // We divide by speed. For now all speeds are the same so no problem. But if this changes this needs to be adjusted.
				}
			}
			return _result;
		}

		//virtual DistMat calculateDepotDistances(std::vector<City>& inCityData, std::vector<Salesperson>& inAgentData) {
		//	size_t _size = inCityData.size();
		//	DistMat _result = new long* [_size];
		//	for (size_t i = 0; i < inCityData.size(); i++)
		//	{
