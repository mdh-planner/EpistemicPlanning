...
#else
	return 18654375; //3165768666 - 142
#endif
}
...
inline getRandomIntegerInRange(T minInclusive, T maxInclusive)
{
	if (minInclusive > maxInclusive) {
		std::cout << "Error: getRandomIntegerInRange() Function: Min is greater than Max!" << std::endl;
		return -1;
	}
...
{
	if (minInclusive > maxInclusive) {
		std::cout << "Error: getRandomRealInRange() Function: Min is greater than Max!" << std::endl;
		return -1;
	}
	auto generator = getMersenneTwisterEngine();
...
inline getRandomIntegerInRangeExcluding(T minInclusive, T maxInclusive, T excluding)
{
	...
	if (maxInclusive == minInclusive) {
		if (excluding == 0) {
			std::cout << "Error: getRandomIntegerInRangeExcluding() Function: Min is equal to Max!" << std::endl;
			return -1;
		}
		else {
			return 0;
